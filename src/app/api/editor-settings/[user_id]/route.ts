...
export const PUT = apiHandler(
  async (request: NextRequest, params: Promise<{ user_id: string }>) => {
    const { user_id } = await params;
    const { settings } = await request.json();

    let editorSettings = await EditorSettings.findOneAndUpdate(
      { user: user_id },
      { $set: { settings: { ...DEFAULT_EDITOR_SETTINGS, ...settings } } },
