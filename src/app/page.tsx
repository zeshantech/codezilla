...
import { useInitializeProblemsStore, useProblemsStore } from "@/store/useProblemsStore";

export default function Page() {
  useInitializeProblemsStore();
  const [activeTab, setActiveTab] = useState("playground");

  const featuredProblems = useProblemsStore((state) => state.featuredProblems);
  const isFeaturedProblemsLoading = useProblemsStore((state) => state.isFeaturedProblemsLoading);
  const isFeaturedProblemsError = useProblemsStore((state) => state.isFeaturedProblemsError);
  const { data: featuredCollections, isLoading: isFeaturedCollectionsLoading, error: isFeaturedCollectionsError } = useFeaturedCollections();
      ...
      <section id="explore" className="py-12">
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-6">
          ...
          <TabsContent value="problems" className="min-h-[400px]">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">{isFeaturedProblemsLoading ? [1, 2, 3].map((i) => <Skeleton key={i} className="h-[220px] rounded-lg"></Skeleton>) : featuredProblems?.map((problem: IProblem) => <ProblemCard key={problem.id} problem={problem} />)}</div>
          ...
          <TabsContent value="collections" className="min-h-[400px]">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">{isFeaturedCollectionsLoading ? [1, 2, 3].map((i) => <Skeleton key={i} className="h-[220px] rounded-lg"></Skeleton>) : featuredCollections?.map((collection: ICollection) => <CollectionCard key={collection.id} collection={collection} />)}</div>
