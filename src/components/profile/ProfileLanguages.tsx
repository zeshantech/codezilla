...
export function ProfileLanguages() {
  ...
  const languages = useMemo(() => {
    return Object.values(languageStats).sort((a, b) => b.percentage - a.percentage);
  }, [languageStats]);
    ...
    <div className="space-y-6">
      <Card>
        <CardHeader>
          ...
          <CardDescription>Languages you've used to solve problems</CardDescription>
        </CardHeader>
        <CardContent>
          <div className="space-y-6">
              ...
              <div key={language.language} className="space-y-2">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <div className={`p-1.5 rounded-md ${languageColors[language.language as ProgrammingLanguageEnum]}`}>{languageIcons[language.language as ProgrammingLanguageEnum]}</div>
                    <div>
                      <div className="font-medium capitalize">{language.language}</div>
                      <div className="text-xs text-muted-foreground">{language.experienceLevel}</div>
                    </div>
                  </div>
                  <div className="text-sm font-medium">{language.problemsSolved} problems</div>
                </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Percentage breakdown of your solved problems by language</CardDescription>
        </CardHeader>
        <CardContent>
            ...
            <div className="relative h-40 w-40">
              <svg className="h-full w-full" viewBox="0 0 100 100" xmlns="http://www.w3.org/2000/svg">
                {languages.map((language, i) => {
                  const percentage = totalSolved > 0 ? (language.problemsSolved / totalSolved) * 100 : 0;
                  // Calculate the pie chart segments
                  let cumulativePercentage = 0;
                  for (let j = 0; j < i; j++) {
                    cumulativePercentage += (languages[j].problemsSolved / totalSolved) * 100;
                  }
                  ...
                  return <path key={language.language} d={path} fill={color} stroke="hsl(var(--background))" strokeWidth="1" />;
                })}
              </svg>
          ...
          <div className="mt-4 flex flex-wrap items-center justify-center gap-3">
            {languages.map((language) => (
              <div key={language.language} className="flex items-center gap-1.5">
                <div
