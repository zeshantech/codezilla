...
export function CreateCollectionDialog() {
  ...
  const { data: allProblems, isLoading: isAllProblemsLoading } =
    useAllProblems();
  const { mutate: createCollection, isPending: isCreatingCollection } =
    useCreateCollection();

  const { watch, setValue, handleSubmit, reset, formState, register } =
    useForm<CreateCollectionFormValues>({
      resolver: zodResolver(
        createCollectionSchema
      ) as Resolver<CreateCollectionFormValues>,
      defaultValues,
    });
              ...
              <div className="flex flex-wrap gap-1 pt-2 border-t">
                {collectionTags.map((tag) => (
                  <Badge
                    key={tag}
                    variant={
                      watch("tags").includes(tag) ? "default" : "outline"
                    }
                    className="cursor-pointer"
                    onClick={() => toggleTag(tag)}
                  >
          ...
          <div className="space-y-2">
            <div className="text-sm font-medium">
              ...
              <span className="ml-2 text-sm text-muted-foreground">
                (Selected: {watch("problems")?.length})
              </span>
            </div>
                ...
                <PopoverTrigger asChild>
                  ...
                  >
                    {watch("problems").length > 0
                      ? `${watch("problems").length} problems selected`
                      ...
                      <CommandGroup>
                        {isAllProblemsLoading ? (
                          ...
                          filteredProblems?.map((problem) => {
                            const isSelected = watch("problems").some(
                              (p) => p.id === problem.id
                            );
                            return (
                              <CommandItem
                    ...
                    {watch("problems").map((problem, index) => (
                      ...
                      >
                        <div className="flex items-center gap-2">
                          <div className="size-5 flex items-center justify-center rounded-full bg-muted text-xs font-medium">
                            {index + 1}
                          </div>
                          <div>
